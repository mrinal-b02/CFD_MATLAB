...
%% Initializing 
% for original collocated mesh/grid
u_final(n_points, n_points) = 0;
v_final(n_points, n_points) = 0;
p_final(n_points, n_points) = 1;
u_final(1,:) = 1;  % the original mesh onto which we had to obtain the results was of size nxn
                   % then staggered pressure, u and v were added of sizes
                   % (n+1)x(n+1), (n+1)xn and nx(n+1) respectively.

% for staggered grid
u(n_points+1, n_points) = 0;
v(n_points, n_points+1) = 0;
p(n_points+1, n_points+1) = 1;
u(1,:) = 2; % this is 'cause the u just inside the boundary(2nd row say) is considered to be 0
            % and u on boundary is 1 so by interpolation the u just outside
            % boundary is 2. (u+0)/2 = 1
            % can also substitute this (2,0) is (1,1) and same results are
            % obtained.
...
while error > error_req
    ...
    for i = 2:n_points
        for j = 2:n_points-1 % this (n_points-1) is 'cause in the interior of domain(5x5 in notebook)
                             % there were 4 rows and 3 columns of u 
                             % so to generalize (2:n_points) for i and
                             % (2:n_points-1) for j
             pressure = -(p(i,j+1) - p(i,j))/h;
             diffusion = (1/Re)*((u(i,j+1) - 2*u(i,j) + u(i,j-1))/(h*h) + (u(i+1,j) - 2*u(i,j) + u(i-1,j))/(h*h));
    ...
    error = 0;
    for i = 2:(n_points-1)
        for j = 2:(n_points-1)% same as above
...
% after solution is converged, staggered variables are merged to collocated
% ones
for i = 1:n_points
    for j = 1:n_points
...
%% Centreline u validation - Comparison with bemchmark
figure(11);
...
figure(11);
hold on
plot(u_ghia, y_ghia, 'ro', 'LineWidth', 2)
xlabel('u')
ylabel('y')
...
figure(22);
hold on 
quiver(X, Y, u_final, v_final, 5, 'w')
